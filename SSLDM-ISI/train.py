...
def main():
    ...
    auto_kl.load_state_dict(torch.load("Autoencoder.pth",map_location=device))# Autoencoder using structural strength information to train loss function
    ...
    myema = EMA(net,beta=0.998,update_every=10).to(device)
    net_optim = optim.Adam(net.parameters(), lr=1e-4)
    
    for epoch in range(301):
        loss_mean = 0.0
        train_loader  = tqdm(train_loader)
        for i,(images_low,images_high,lable,names) in enumerate(train_loader):
            ...
            with autocast(device_type="cuda",dtype=torch.float16):
                with torch.no_grad():
                    ...
                    input = auto_kl(lable)# 32 32 4
                    input = input*0.3242
                input = einops.rearrange(input,"(b f) c w h -> b c f w h",f=16)
                loss = net(input,[images_low,images_high_latent,images_high])
            net_optim.zero_grad()
            scaler.scale(loss).backward()
            scaler.step(net_optim)
            scaler.update()
            current_lr = net_optim.param_groups[0]['lr']
            loss_mean = loss_mean+loss.item()
            train_loader.desc="epoch: {} loss_mean: {} lr: {}".format(epoch,round(loss_mean/(i+1),3),current_lr)
            myema.update()
            ...
            if (epoch % 5 == 0):
                    ...
                    with autocast(device_type="cuda",dtype=torch.float16):
                        with torch.no_grad():
                            auto_kl.eval()
                            images_high_latent = auto_kl(images_high)
                    sampled_images = myema.ema_model.sample(batch_size=1,cond=[images_low,images_high_latent,images_high])
                    ...
                    video_tensor_to_gif(torch.cat([sampled_images,lable],dim=2), os.path.join("result", f"{epoch}.gif"))
            torch.save(myema.ema_model.state_dict(),"pth/"+f"{epoch}.pth")
main()